...
class MockDefaultMQPushConsumer : public DefaultMQPushConsumer {
 public:
    MockDefaultMQPushConsumer(const string& groupname)
...
TEST(cPushComsumer, infomock) {
    ...
    MockDefaultMQPushConsumer *pushComsumer = new MockDefaultMQPushConsumer(
            "testGroup");
...
TEST(cPushComsumer, info) {
    ...
    DefaultMQPushConsumer *mqPushConsumer =
            (DefaultMQPushConsumer *) cpushConsumer;
    ...
    EXPECT_EQ(SetPushConsumerNameServerAddress(cpushConsumer, "127.0.0.1:9876"),
              OK);
    ...
    EXPECT_EQ(
            SetPushConsumerSessionCredentials(cpushConsumer, "accessKey",
                                              "secretKey", "channel"),
            OK);
    SessionCredentials sessionCredentials = mqPushConsumer
...
TEST(cPushComsumer, null) {
    ...
    EXPECT_EQ(RegisterMessageCallbackOrderly( cpushConsumer, NULL),
              NULL_POINTER);
    ...
    EXPECT_EQ(SetPushConsumerLogLevel( NULL , E_LOG_LEVEL_LEVEL_NUM ),
              NULL_POINTER);
