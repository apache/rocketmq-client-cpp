...
class MockDefaultMQPullConsumer : public DefaultMQPullConsumer {
 public:
    MockDefaultMQPullConsumer(const string& groupname)
...
TEST(cpullConsumer , pull) {
    ...
    MockDefaultMQPullConsumer *mqPullConsumer = new MockDefaultMQPullConsumer(
            "groudId");
    ...
    EXPECT_CALL(*mqPullConsumer , pull(_,_,_,_)).WillOnce(
            Return(timeOutPullResult)).WillOnce(Return(noNewMsgPullResult))
            .WillOnce(Return(noMatchedMsgPullResult)).WillOnce(
            Return(offsetIllegalPullResult)).WillOnce(Return(defaultPullResult))
            /*.WillOnce(Return(timeOutPullResult))*/.WillOnce(
            Return(foundPullResult));

    CPullResult timeOutcPullResult = Pull(pullConsumer, cMessageQueue, "123123",
                                          0, 0);
    EXPECT_EQ(timeOutcPullResult.pullStatus, E_BROKER_TIMEOUT);

    CPullResult noNewMsgcPullResult = Pull(pullConsumer, cMessageQueue,
                                           "123123", 0, 0);
    EXPECT_EQ(noNewMsgcPullResult.pullStatus, E_NO_NEW_MSG);

    CPullResult noMatchedMsgcPullResult = Pull(pullConsumer, cMessageQueue,
                                               "123123", 0, 0);
    EXPECT_EQ(noMatchedMsgcPullResult.pullStatus, E_NO_MATCHED_MSG);

    CPullResult offsetIllegalcPullResult = Pull(pullConsumer, cMessageQueue,
                                                "123123", 0, 0);
    EXPECT_EQ(offsetIllegalcPullResult.pullStatus, E_OFFSET_ILLEGAL);

    CPullResult defaultcPullResult = Pull(pullConsumer, cMessageQueue, "123123",
                                          0, 0);
    EXPECT_EQ(defaultcPullResult.pullStatus, E_NO_NEW_MSG);

    CPullResult exceptionPullResult = Pull(pullConsumer, cMessageQueue, NULL, 0,
                                           0);
    EXPECT_EQ(exceptionPullResult.pullStatus, E_BROKER_TIMEOUT);

    CPullResult foundcPullResult = Pull(pullConsumer, cMessageQueue, "123123",
                                        0, 0);
...
TEST(cpullConsumer , infoMock) {
    ...
    EXPECT_CALL(*mqPullConsumer,fetchSubscribeMessageQueues(_,_)).Times(1)
            .WillOnce(SetArgReferee<1>(fullMQ));
...
TEST(cpullConsumer, init) {
    ...
    EXPECT_EQ(GetPullConsumerGroupID(pullConsumer),
              defaultMQPullConsumer->getGroupName().c_str());

    EXPECT_EQ(SetPullConsumerNameServerAddress(pullConsumer, "127.0.0.1:10091"),
              OK);
    ...
    EXPECT_EQ(
            SetPullConsumerSessionCredentials(pullConsumer, "accessKey",
                                              "secretKey", "channel"),
            OK);
    SessionCredentials sessionCredentials = defaultMQPullConsumer
    ...
    EXPECT_EQ(SetPullConsumerLogLevel(pullConsumer, E_LOG_LEVEL_DEBUG),
              OK);
}
