...
TEST(cmessageExt , info) {
    ...
    EXPECT_EQ(GetMessageProperty(messageExt, "testKey"),
              mqMessageExt->getProperty("testKey"));

    mqMessageExt->setMsgId("msgId123456");
    EXPECT_EQ(GetMessageId(messageExt), mqMessageExt->getMsgId());

    mqMessageExt->setDelayTimeLevel(1);
    EXPECT_EQ(GetMessageDelayTimeLevel(messageExt),
              mqMessageExt->getDelayTimeLevel());
    ...
    EXPECT_EQ(GetMessageReconsumeTimes(messageExt),
              mqMessageExt->getReconsumeTimes());
    ...
    EXPECT_EQ(GetMessageBornTimestamp(messageExt),
              mqMessageExt->getBornTimestamp());

    mqMessageExt->setStoreTimestamp(123123);
    EXPECT_EQ(GetMessageStoreTimestamp(messageExt),
              mqMessageExt->getStoreTimestamp());

    mqMessageExt->setQueueOffset(1024);
    EXPECT_EQ(GetMessageQueueOffset(messageExt),
              mqMessageExt->getQueueOffset());

    mqMessageExt->setCommitLogOffset(2048);
    EXPECT_EQ(GetMessageCommitLogOffset(messageExt),
              mqMessageExt->getCommitLogOffset());

    mqMessageExt->setPreparedTransactionOffset(4096);
    EXPECT_EQ(GetMessagePreparedTransactionOffset(messageExt),
              mqMessageExt->getPreparedTransactionOffset());
}
