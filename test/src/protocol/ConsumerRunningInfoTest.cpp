...
TEST(consumerRunningInfo, init) {
    ...
    map<MessageQueue, ProcessQueueInfo> mqTable =
            consumerRunningInfo.getMqTable();
    EXPECT_EQ(mqTable[messageQueue].commitOffset, processQueueInfo.commitOffset);

    // encode start
    consumerRunningInfo.setProperty(ConsumerRunningInfo::PROP_NAMESERVER_ADDR,
                                    "127.0.0.1:9876");
    consumerRunningInfo.setProperty(
            ConsumerRunningInfo::PROP_THREADPOOL_CORE_SIZE, "core_size");
    consumerRunningInfo.setProperty(ConsumerRunningInfo::PROP_CONSUME_ORDERLY,
                                    "consume_orderly");
    consumerRunningInfo.setProperty(ConsumerRunningInfo::PROP_CONSUME_TYPE,
                                    "consume_type");
    consumerRunningInfo.setProperty(ConsumerRunningInfo::PROP_CLIENT_VERSION,
                                    "client_version");
    consumerRunningInfo.setProperty(
            ConsumerRunningInfo::PROP_CONSUMER_START_TIMESTAMP, "127");
    //TODO
