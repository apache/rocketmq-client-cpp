...
TEST(validators, regularExpressionMatcher) {
    ...
    EXPECT_TRUE(
            Validators::regularExpressionMatcher(string("123456"), string()));

    EXPECT_TRUE(
            Validators::regularExpressionMatcher(string("123456"),
                                                 string("123")));
}
...
TEST(validators, checkMessage) {
    ...
    EXPECT_THROW(Validators::checkMessage(MQMessage("testTopic", string()), 1),
                 MQClientException);

    EXPECT_THROW(
            Validators::checkMessage(MQMessage("testTopic", string("123")), 2),
            MQClientException);
}
