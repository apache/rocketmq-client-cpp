...
class MockClientRemotingProcessor : public ClientRemotingProcessor {
 public:
    MockClientRemotingProcessor(MQClientFactory* factrory)
            : ClientRemotingProcessor(factrory) {
...
class MockMQClientFactory : public MQClientFactory {
 public:
    MockMQClientFactory(const string& clientID, int pullThreadNum,
                        uint64_t tcpConnectTimeout,
                        uint64_t tcpTransportTryLockTimeout, string unitName)
            : MQClientFactory(clientID, pullThreadNum, tcpConnectTimeout,
                              tcpTransportTryLockTimeout, unitName) {
    }

    MOCK_METHOD3(resetOffset, void(const string& group, const string& topic,const map<MQMessageQueue, int64>& offsetTable));
    MOCK_METHOD1(consumerRunningInfo, ConsumerRunningInfo*(const string& consumerGroup));
    MOCK_METHOD2(getSessionCredentialFromConsumer, bool(const string& consumerGroup, SessionCredentials& sessionCredentials));
    MOCK_METHOD1(doRebalanceByConsumerGroup, void(const string& consumerGroup));
};

TEST(clientRemotingProcessor, processRequest) {
    MockMQClientFactory* factory = new MockMQClientFactory("testClientId", 4,
                                                           3000, 4000, "a");
    ...
    EXPECT_TRUE(clientRemotingProcessor.processRequest(addr, command)== NULL);
    ...
    RemotingCommand* twoCommand = new RemotingCommand(
            MQRequestCode::NOTIFY_CONSUMER_IDS_CHANGED, header);
...
TEST(clientRemotingProcessorS, getConsumerRunningInfo) {
    ...
    EXPECT_CALL( *factory ,consumerRunningInfo(_) ).Times(2).WillOnce(
            Return(info)).WillOnce(Return(info));
    EXPECT_CALL( *factory ,getSessionCredentialFromConsumer(_,_) ).Times(2);  //.WillRepeatedly(SetArgReferee<1>(sessionCredentials));
    ClientRemotingProcessor clientRemotingProcessor(factory);
    ...
    // 这里可以调用，不知道为什么，到 clientRemotingProcessor.getConsumerRunningInfo就调用mock失败
    //info = factory->consumerRunningInfo("1231");
    //SessionCredentials sessionCredentials;
    RemotingCommand* command = clientRemotingProcessor.getConsumerRunningInfo(
            "127.0.0.1:9876", request);
    EXPECT_EQ(command->getCode(), MQResponseCode::SYSTEM_ERROR);
    EXPECT_EQ(command->getRemark(),
              "The Consumer Group not exist in this consumer");

    //command = clientRemotingProcessor.getConsumerRunningInfo("127.0.0.1:9876",request);
    //EXPECT_EQ(command->getCode(), MQResponseCode::SUCCESS_VALUE);

    //EXPECT_EQ(command->getMsgBody(), info.encode());
