...
class MockAsyncCallbackWrap : public SendCallbackWrap {
 public:
    MockAsyncCallbackWrap(AsyncCallback* pAsyncCallback,
    ...
    asyncCallBackType getCallbackType() {
        return asyncCallBackType::sendCallbackWrap;
    }
...
TEST(responseFuture, init) {
    ...
    EXPECT_TRUE(responseFuture.getAsyncCallbackWrap() == NULL);

    AsyncCallback* pAsyncCallback = NULL;
    MQClientAPIImpl* pclientAPI = NULL;
    SendCallbackWrap* pcall = new SendCallbackWrap("", MQMessage(),
                                                   pAsyncCallback, pclientAPI);
    ResponseFuture twoResponseFuture(13, 4, NULL, 1000, true, pcall);
    ...
    EXPECT_FALSE(twoResponseFuture.getAsyncCallbackWrap() == NULL);
...
TEST(responseFuture, executeInvokeCallback) {
    ...
    MQClientAPIImpl* pclientAPI;
    MockAsyncCallbackWrap* wrap = new MockAsyncCallbackWrap(pAsyncCallback,
                                                            pclientAPI);

    TcpRemotingClient* client;
    ResponseFuture responseFuture(13, 4, client, 1000, false, wrap);
    ...
    responseFuture.setResponse(pResponseCommand);
    responseFuture.setAsyncCallBackStatus(
            asyncCallBackStatus::asyncCallBackStatus_response);
    responseFuture.executeInvokeCallback();
    EXPECT_EQ(pResponseCommand->getCode(), 0);

    ResponseFuture twoResponseFuture(13, 4, client, 1000, false, NULL);
    pResponseCommand = new RemotingCommand();
...
TEST(responseFuture, executeInvokeCallbackException) {
    ...
    responseFuture.setAsyncCallBackStatus(
            asyncCallBackStatus::asyncCallBackStatus_timeout);
    responseFuture.executeInvokeCallbackException();

    ResponseFuture twoRresponseFuture(13, 4, client, 1000, false, NULL);
