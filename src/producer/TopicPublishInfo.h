...
class TopicPublishInfo {
 public:
  TopicPublishInfo() : m_sendWhichQueue(0), m_scheduledExecutorService(false) {
    m_scheduledExecutorService.startup();
    m_scheduledExecutorService.schedule(std::bind(&TopicPublishInfo::op_resumeNonServiceMessageQueueList, this), 60,
                                        time_unit::seconds);
  }
  ...
  void op_resumeNonServiceMessageQueueList() {
  ...
  int getWhichQueue() { return m_sendWhichQueue.load(std::memory_order_acquire); }
      ...
      if (!lastmq.getBrokerName().empty()) {
        for (size_t i = 0; i < m_queues.size(); i++) {
          ...
          LOG_DEBUG("lastmq broker not empty, m_sendWhichQueue:%d, pos:%d",
                    m_sendWhichQueue.load(std::memory_order_acquire), pos);
        ...
        ++m_sendWhichQueue;
        LOG_DEBUG("lastmq broker empty, m_sendWhichQueue:%d, pos:%d", m_sendWhichQueue.load(std::memory_order_acquire),
                  pos);
        mq_index = pos;
