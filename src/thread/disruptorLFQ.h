...
public:
    disruptorLFQ(int threadCount) {
        ...
        m_ring_buffer.reset(
            new RingBuffer<Task>(m_task_factory.get(), 1024,  // default size is 1024, must be n power of 2
                                 kSingleThreadedStrategy, kBlockingStrategy));  // load normal, lowest CPU occupy, but
