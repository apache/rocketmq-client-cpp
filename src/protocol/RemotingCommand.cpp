...
RemotingCommand::RemotingCommand(int code, string language, int version, int opaque, int flag, string remark,
...
RemotingCommand::RemotingCommand(const RemotingCommand& command) { Assign(command); }
...
CommandHeader* RemotingCommand::getCommandHeader() const { return m_pExtHeader.get(); }
...
void RemotingCommand::addExtField(const string& key, const string& value) { m_extFields[key] = value; }
