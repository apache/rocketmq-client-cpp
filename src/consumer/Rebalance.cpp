...
map<string, SubscriptionData*>& Rebalance::getSubscriptionInner() { return m_subscriptionData; }

void Rebalance::setSubscriptionData(const string& topic, SubscriptionData* pdata) {
    if (pdata != NULL && m_subscriptionData.find(topic) == m_subscriptionData.end()) m_subscriptionData[topic] = pdata;
}
    ...
    for (map<string, vector<MQMessageQueue>*>::iterator itb = brokerMqs.begin(); itb != brokerMqs.end(); ++itb) {
        ...
        try {
            m_pClientFactory->getMQClientAPIImpl()->unlockBatchMQ(
                pFindBrokerResult->brokerAddr, unlockBatchRequest.get(), 1000, m_pConsumer->getSessionCredentials());
...
void RebalancePull::messageQueueChanged(const string& topic, vector<MQMessageQueue>& mqAll,
            ...
            if (mqsSelf.empty() || (find(mqsSelf.begin(), mqsSelf.end(), mqtemp) == mqsSelf.end())) {
                if (!(it->second->isDroped())) {
                    ...
                    it->second->clearAllMsgs();  // add clear operation to avoid bad state
                                                 ...
                                                 // normal
                    LOG_INFO("drop mq:%s, delete opaque:%d", mqtemp.toString().c_str(),
                             it->second->getLatestPullRequestOpaque());
                }
                changed = true;
            }
        }
        ...
        if (pPullRequest && pPullRequest->isDroped()) {
            ...
            if (nextOffset >= 0) {
                ...
                */
                ...
                LOG_ERROR("get fatel error QueryOffset of mq:%s, do not reconsume this queue",
                          (it2->toString()).c_str());
            }
        }
            ...
            if (lastOffset >= 0) {
                ...
                if (UtilAll::startsWith_retry(mq.getTopic())) {
                    ...
                    try {
                        ...
                        LOG_ERROR("CONSUME_FROM_TIMESTAMP error, lastOffset  of mq:%s, return 0",
                                  mq.toString().c_str());
                        result = -1;
                    }
                }
...
void RebalancePush::messageQueueChanged(const string& topic, vector<MQMessageQueue>& mqAll,
                                        vector<MQMessageQueue>& mqDivided) {}
