        ...
        case CREATE_JUST: {
            ...
            if (m_pMessageListener) {
                if (m_pMessageListener->getMessageListenerType() == messageListenerOrderly) {
                    ...
                    m_consumerService =
                        new ConsumeMessageOrderlyService(this, m_consumeThreadCount, m_pMessageListener);
                        ...
                        // are allocating ConsumeMessageConcurrentlyService
                {
                    ...
                    m_consumerService =
                        new ConsumeMessageConcurrentlyService(this, m_consumeThreadCount, m_pMessageListener);
                }
            }
            ...
            if (!registerOK) {
                ...
                THROW_MQEXCEPTION(MQClientException, "The cousumer group[" + getGroupName() +
...
ConsumeMsgService *DefaultMQPushConsumer::getConsumerMsgService() const { return m_consumerService; }

OffsetStore *DefaultMQPushConsumer::getOffsetStore() const { return m_pOffsetStore; }
...
ConsumeType DefaultMQPushConsumer::getConsumeType() { return CONSUME_PASSIVELY; }
...
void DefaultMQPushConsumer::removeConsumeOffset(const MQMessageQueue &mq) { m_pOffsetStore->removeOffset(mq); }
...
void DefaultMQPushConsumer::runPullMsgQueue(TaskQueue *pTaskQueue) { pTaskQueue->run(); }
    ...
    try {
        ...
        switch (pullResult.pullStatus) {
            ...
            case NO_NEW_MSG: {
                ...
                producePullMsgTask(request);
                LOG_DEBUG("NO_NEW_MSG:%s,nextBeginOffset:%lld", messageQueue.toString().c_str(),
...
int DefaultMQPushConsumer::getConsumeThreadCount() const { return m_consumeThreadCount; }

void DefaultMQPushConsumer::setPullMsgThreadPoolCount(int threadCount) { m_pullMsgThreadPoolNum = threadCount; }

int DefaultMQPushConsumer::getPullMsgThreadPoolCount() const { return m_pullMsgThreadPoolNum; }

int DefaultMQPushConsumer::getConsumeMessageBatchMaxSize() const { return m_consumeMessageBatchMaxSize; }

void DefaultMQPushConsumer::setConsumeMessageBatchMaxSize(int consumeMessageBatchMaxSize) {
    if (consumeMessageBatchMaxSize >= 1) m_consumeMessageBatchMaxSize = consumeMessageBatchMaxSize;
}
...
int DefaultMQPushConsumer::getMaxCacheMsgSizePerQueue() const { return m_maxMsgCacheSize; }
