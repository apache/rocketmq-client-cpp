        ...
        if (ifs.is_open()) {
            if (ifs.peek() != std::ifstream::traits_type::eof()) {
                ...
                try {
                    ...
                    errorMsg.append(m_storeFile).append(
                ...
                for (map<string, int64>::iterator it = m_offsetTable_tmp.begin(); it != m_offsetTable_tmp.end(); ++it) {
                    ...
                    MQMessageQueue mq(object["topic"].asString(), object["brokerName"].asString(),
...
int64 LocalFileOffsetStore::readOffset(const MQMessageQueue& mq, ReadOffsetType type,
...
int64 RemoteBrokerOffsetStore::readOffset(const MQMessageQueue& mq, ReadOffsetType type,
                                          const SessionCredentials& session_credentials) {
    switch (type) {
        case MEMORY_FIRST_THEN_STORE:
        case READ_FROM_MEMORY: {
...
void RemoteBrokerOffsetStore::updateConsumeOffsetToBroker(const MQMessageQueue& mq, int64 offset,
                                                          const SessionCredentials& session_credentials) {
