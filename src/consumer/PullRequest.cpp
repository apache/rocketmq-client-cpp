...
int64 PullRequest::removeMessage(vector<MQMessageExt>& msgs) {
    ...
    if (!m_msgTreeMap.empty()) {
        ...
        if (!m_msgTreeMap.empty()) {
            map<int64, MQMessageExt>::iterator it = m_msgTreeMap.begin();
            result = it->first;
            LOG_INFO("cache msg size:" SIZET_FMT " of pullRequest:%s, return offset result is:%lld",
...
void PullRequest::setLastLockTimestamp(int64 time) { m_lastLockTimestamp = time; }

int64 PullRequest::getLastLockTimestamp() const { return m_lastLockTimestamp; }

void PullRequest::setLastPullTimestamp(uint64 time) { m_lastPullTimestamp = time; }

uint64 PullRequest::getLastPullTimestamp() const { return m_lastPullTimestamp; }

void PullRequest::setLastConsumeTimestamp(uint64 time) { m_lastConsumeTimestamp = time; }
...
boost::timed_mutex& PullRequest::getPullRequestCriticalSection() { return m_consumeLock; }
