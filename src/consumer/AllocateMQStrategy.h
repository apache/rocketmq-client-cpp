...
public:
    ...
    virtual void allocate(const std::string& currentCID, std::vector<MQMessageQueue>& mqAll,
                          std::vector<std::string>& cidAll, std::vector<MQMessageQueue>& outReuslt) = 0;
};
...
public:
    ...
    virtual void allocate(const std::string& currentCID, std::vector<MQMessageQueue>& mqAll,
                          std::vector<std::string>& cidAll, std::vector<MQMessageQueue>& outReuslt) {
        outReuslt.clear();
        if (currentCID.empty()) {
        ...
        int averageSize = mqAllSize <= cidAllSize ? 1 : (mod > 0 && index < mod ? mqAllSize / cidAllSize + 1
