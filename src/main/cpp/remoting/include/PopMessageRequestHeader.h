...
public:
  ...
  void consumerGroup(absl::string_view consumer_group) {
    consumer_group_ =
        std::string(consumer_group.data(), consumer_group.length());
  }
  ...
  void batchSize(std::int32_t batch_size) { max_message_number_ = batch_size; }

  void invisibleTime(std::int64_t time) { invisible_time_ = time; }
