...
ClientManagerPtr ClientManagerFactory::getClientManager(const ClientConfigImpl& client_config) {
  {
    absl::MutexLock lock(&client_manager_table_mtx_);
    auto search = client_manager_table_.find(client_config);
    if (search != client_manager_table_.end()) {
      ...
      if (client_manager) {
        ...
        client_manager_table_.erase(client_config);
      }
    }
    ClientManagerPtr client_manager = std::make_shared<ClientManagerImpl>(client_config);
    std::weak_ptr<ClientManager> client_instance_weak_ptr(client_manager);
    client_manager_table_.insert_or_assign(client_config, client_instance_weak_ptr);
    SPDLOG_INFO("Created a new client manager[resource_namespace={}]", client_config.resourceNamespace());
    return client_manager;
  }
}

void ClientManagerFactory::addClientManager(const ClientConfigImpl& client_config,
                                            const ClientManagerPtr& client_manager) {
  absl::MutexLock lk(&client_manager_table_mtx_);
  client_manager_table_.insert_or_assign(client_config, std::weak_ptr<ClientManager>(client_manager));
}
