...
ClientConfig::ClientConfig(std::string group_name) : group_name_(std::move(group_name)), io_timeout_(absl::Seconds(3)) {
  instance_name_ = "DEFAULT";
}

std::string ClientConfig::clientId() const {
  std::stringstream ss;
  ss << UtilAll::getHostIPv4();
  ss << "@";
...
CredentialsProviderPtr ClientConfig::credentialsProvider() {
