...
RemotingCommand* ClientRemotingProcessor::resetOffset(RemotingCommand* request) {
    ...
    if (pbody->getSize()) {
        ...
        if (offsetBody) {
            m_mqClientFactory->resetOffset(offsetHeader->getGroup(), offsetHeader->getTopic(),
...
std::map<MQMessageQueue, int64> ResetOffsetBody::getOffsetTable() { return m_offsetTable; }

void ResetOffsetBody::setOffsetTable(MQMessageQueue mq, int64 offset) { m_offsetTable[mq] = offset; }
