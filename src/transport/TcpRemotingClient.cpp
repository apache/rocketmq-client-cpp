...
RemotingCommand* TcpRemotingClient::invokeSync(const string& addr, RemotingCommand& request,
...
bool TcpRemotingClient::invokeAsync(const string& addr, RemotingCommand& request, AsyncCallbackWrap* cbw,
                                    int64 timeoutMilliseconds, int maxRetrySendTimes, int retrySendTimes) {
    boost::shared_ptr<TcpTransport> pTcp = GetTransport(addr, true);
    if (pTcp != NULL) {
...
boost::shared_ptr<TcpTransport> TcpRemotingClient::CreateNameserverTransport(bool needRespons) {
    ...
    if (bGetMutex) {
        ...
        for (; itp != m_namesrvAddrList.end(); ++itp) {
            ...
            if (m_namesrvIndex == numeric_limits<unsigned int>::max()) m_namesrvIndex = 0;
            m_namesrvIndex++;
...
void TcpRemotingClient::registerProcessor(MQRequestCode requestCode, ClientRemotingProcessor* clientRemotingProcessor) {
    if (m_requestTable.find(requestCode) != m_requestTable.end()) m_requestTable.erase(requestCode);
