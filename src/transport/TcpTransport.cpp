...
void TcpTransport::setTcpConnectStatus(tcpConnectStatus connectStatus) { m_tcpConnectStatus = connectStatus; }

tcpConnectStatus TcpTransport::getTcpConnectStatus() { return m_tcpConnectStatus; }
...
void TcpTransport::readNextMessageIntCallback(struct bufferevent *bev, void *ctx) {
    ...
    while (1) {
        ...
        LOG_DEBUG("fd:%d, totalLen:" SIZET_FMT ", bytesInMessage:%d", bufferevent_getfd(bev), v[0].iov_len,
                  bytesInMessage);
