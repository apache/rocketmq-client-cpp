...
namespace rocketmq {
//<!************************************************************************
ResponseFuture::ResponseFuture(int requestCode, int opaque, TcpRemotingClient* powner, int64 timeout,
                               bool bAsync /* = false */, AsyncCallbackWrap* pcall /* = NULL */) {
...
void ResponseFuture::setSendRequestOK(bool sendRequestOK) { m_sendRequestOK = sendRequestOK; }
...
void ResponseFuture::executeInvokeCallback() {
    if (m_pCallbackWrap == NULL) {
        ...
        if (m_asyncCallbackStatus == asyncCallBackStatus_response) {
            ...
            if (m_pResponseCommand) deleteAndZero(m_pResponseCommand);  // the responseCommand from
...
int ResponseFuture::getMaxRetrySendTimes() const { return m_maxRetrySendTimes; }
int ResponseFuture::getRetrySendTimes() const { return m_retrySendTimes; }

void ResponseFuture::setMaxRetrySendTimes(int maxRetryTimes) { m_maxRetrySendTimes = maxRetryTimes; }
void ResponseFuture::setRetrySendTimes(int retryTimes) { m_retrySendTimes = retryTimes; }

void ResponseFuture::setBrokerAddr(const std::string& brokerAddr) { m_brokerAddr = brokerAddr; }
void ResponseFuture::setRequestCommand(const RemotingCommand& requestCommand) { m_requestCommand = requestCommand; }

const RemotingCommand& ResponseFuture::getRequestCommand() { return m_requestCommand; }
std::string ResponseFuture::getBrokerAddr() const { return m_brokerAddr; }
...
RemotingCommand* ResponseFuture::getCommand() const { return m_pResponseCommand; }

AsyncCallbackWrap* ResponseFuture::getAsyncCallbackWrap() { return m_pCallbackWrap; }
