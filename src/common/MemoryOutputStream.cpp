...
MemoryOutputStream::MemoryOutputStream(MemoryBlock& memoryBlockToWriteTo, const bool appendToExistingBlockContent)
    ...
    if (appendToExistingBlockContent) position = size = memoryBlockToWriteTo.getSize();
}
...
void MemoryOutputStream::trimExternalBlockSize() {
    if (blockToUse != &internalBlock && blockToUse != NULL) blockToUse->setSize(size, false);
}
...
const void* MemoryOutputStream::getData() const {
    ...
    if ((unsigned int)blockToUse->getSize() > size) static_cast<char*>(blockToUse->getData())[size] = 0;
...
int64 MemoryOutputStream::writeFromInputStream(InputStream& source, int64 maxNumBytesToWrite) {
    ...
    if (availableData > 0) {
        if (maxNumBytesToWrite > availableData || maxNumBytesToWrite < 0) maxNumBytesToWrite = availableData;
