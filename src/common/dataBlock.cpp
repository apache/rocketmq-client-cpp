...
//==============================================================================
bool MemoryBlock::operator==(const MemoryBlock &other) const { return matches(other.data, other.size); }

bool MemoryBlock::operator!=(const MemoryBlock &other) const { return !operator==(other); }
    ...
    if (size != newSize) {
        if (newSize <= 0) {
            ...
            if (data != NULL) {
                ...
                if (initialiseToZero && (newSize > size)) memset(data + size, 0, newSize - size);
            } else {
                std::free(data);
