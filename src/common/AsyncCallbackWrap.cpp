...
//<!************************************************************************
SendCallbackWrap::SendCallbackWrap(const string& brokerName, const MQMessage& msg, AsyncCallback* pAsyncCallback,
...
void SendCallbackWrap::operationComplete(ResponseFuture* pResponseFuture, bool bProducePullRequest) {
    ...
    if (!pResponse) {
        ...
        try {
            ...
            if (pCallback) {
                LOG_DEBUG(
                    "operationComplete: processSendResponse success, opaque:%d, maxRetryTime:%d, retrySendTimes:%d",
            ...
            if (pResponseFuture->getASyncFlag() && retryTimes < maxRetryTimes && maxRetryTimes > 1) {
                ...
                retryTimes += 1;
                LOG_WARN(
                    "retry send, opaque:%d, sendTimes:%d, maxRetryTimes:%d, left_timeout:%lld, brokerAddr:%s, msg:%s",
                ...
                try {
                    m_pClientAPI->sendMessageAsync(pResponseFuture->getBrokerAddr(), m_brokerName, m_msg,
                                                   (RemotingCommand&)requestCommand, pCallback, left_timeout_ms,
    ...
    if (!pResponse) {
        ...
        try {
            if (m_pArg.pPullWrapper) {
                ...
                PullResult result =
                    m_pArg.pPullWrapper->processPullResult(m_pArg.mq, pullResult.get(), &m_pArg.subData);
                if (pCallback) pCallback->onSuccess(m_pArg.mq, result, bProducePullRequest);
            } else {
