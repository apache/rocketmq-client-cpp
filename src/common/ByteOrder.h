...
inline uint16 ByteOrder::swap(uint16 n) { return static_cast<uint16>((n << 8) | (n >> 8)); }

inline uint32 ByteOrder::swap(uint32 n) { return (n << 24) | (n >> 24) | ((n & 0xff00) << 8) | ((n & 0xff0000) >> 8); }
...
inline uint64 ByteOrder::littleEndianInt64(const void* const bytes) { return swap(*static_cast<const uint64*>(bytes)); }
